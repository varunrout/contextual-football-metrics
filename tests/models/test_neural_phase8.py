...
def _synthetic_frames(shots: pd.DataFrame, players_per_event: int = 14, seed: int = 1) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    rows = []
    for eid, sx, sy in zip(shots["event_internal_id"], shots["x_location"], shots["y_location"]):
...
class TestSetTransformerCxG:
    ...
    def trained_model(self, tmp_path):
        ...
        model = SetTransformerCxGModel(
            ...
            max_epochs=2, batch_size=16, random_state=0,
            device="cpu",
        )
...
class TestLadderIntegration:
    def test_ladder_includes_neural_when_flag_set(self, tmp_path):
        ...
        ladder = CxGLadder()
        results = ladder.run(
            shots, target_col="goal", match_id_col="match_id",
            n_folds=2, include_360=False, include_neural=True,
