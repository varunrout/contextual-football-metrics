...
def _home_away_lookup(matches_df: pd.DataFrame | None) -> pd.DataFrame:
    if matches_df is None or matches_df.empty:
        return pd.DataFrame(columns=["match_internal_id", "team_internal_id", "home_or_away", "knockout_or_group"])
...
def build_match_context_features(
    ...
    cols = ["internal_id", "match_internal_id", "team_internal_id",
            "timestamp", "period", "index", "goal"]
    ...
    work["_team_cum"] = (
        work.groupby(["match_internal_id", "team_internal_id"])["_goal"]
        .cumsum()
        .sub(work["_goal"])
    )
    # Goals before this event in the whole match (all teams)
    work["_match_cum"] = (
        work.groupby("match_internal_id")["_goal"]
        .cumsum()
        .sub(work["_goal"])
    )
    ...
    if not rest.empty:
        ...
        if matches_df is not None and not matches_df.empty and {
            ...
            opp_map = pd.concat([
                matches_df[["internal_id", "home_team_internal_id", "away_team_internal_id"]].rename(
                    columns={"internal_id": "match_internal_id",
                             "home_team_internal_id": "team_internal_id",
                             "away_team_internal_id": "_opp_team"}),
                matches_df[["internal_id", "away_team_internal_id", "home_team_internal_id"]].rename(
                    columns={"internal_id": "match_internal_id",
            ...
            for _, r in pairs.iterrows():
                ...
                if len(teams) >= 2:
                    opp_rows += [
                        {"match_internal_id": r["match_internal_id"], "team_internal_id": teams[0], "_opp_team": teams[1]},
                        {"match_internal_id": r["match_internal_id"], "team_internal_id": teams[1], "_opp_team": teams[0]},
                    ]
            opp_map = pd.DataFrame(opp_rows)

        opp_rest = opp_map.merge(
            rest.rename(columns={"team_internal_id": "_opp_team", "rest_days": "opponent_rest_days"}),
