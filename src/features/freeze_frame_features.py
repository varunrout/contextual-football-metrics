...
def build_freeze_frame_features(events_df: pd.DataFrame, frames_df: pd.DataFrame) -> pd.DataFrame:
    ...
    frames_grouped = (
        frames_df.groupby("event_internal_id")
        if not frames_df.empty and "event_internal_id" in frames_df.columns
        else None
    )
    ...
    for i, row in event_sort.iterrows():
        ...
        opp_distances = [
            _distance(ball_x, ball_y, float(x), float(y))
            for x, y in zip(opponents["x"], opponents["y"], strict=False)
        ]
        opp_distances_sorted = sorted(opp_distances)
        out["nearest_defender_distance"] = (
            opp_distances_sorted[0] if opp_distances_sorted else np.nan
        )
        out["second_nearest_defender_distance"] = (
            opp_distances_sorted[1] if len(opp_distances_sorted) > 1 else np.nan
        )
        ...
        in_box = opponents[
            (opponents["x"] >= BOX_X_MIN) & (opponents["y"].between(BOX_Y_MIN, BOX_Y_MAX))
        ]
        in_central = opponents[
            (opponents["x"] >= 70.0) & (opponents["y"].between(CENTRAL_Y_MIN, CENTRAL_Y_MAX))
        ]
        ...
        if not gk.empty:
            ...
            out["keeper_angle_coverage"] = max(
                0.0, min(1.0, 1.0 - (out["keeper_distance_to_shooter"] / 30.0))
            )

        lane_blocks = 0
        for _, opp in opponents.iterrows():
            d = _point_to_segment_distance(
                float(opp["x"]), float(opp["y"]), ball_x, ball_y, GOAL_X, GOAL_Y
            )
            if d <= 1.0:
                lane_blocks += 1
        ...
        if i + 1 < len(event_sort):
            ...
            if same_possession:
                ...
                if frames_grouped is not None and next_event_id in frames_grouped.groups:
                    ...
                    next_y = pd.to_numeric(
                        nxt.get("y_location", nxt.get("y", float("nan"))), errors="coerce"
                    )
                    next_x = float(next_x) if pd.notna(next_x) else float("nan")
                    next_y = float(next_y) if pd.notna(next_y) else float("nan")
                    out["defenders_between_ball_and_goal_after_action"] = int(
                        (opp_next["x"] > next_x).sum()
                    )
                    next_d3 = [
                        _distance(next_x, next_y, float(x), float(y))
                        for x, y in zip(opp_next["x"], opp_next["y"], strict=False)
                    ]
                    out["pressure_after_action"] = float(sum(d <= 3.0 for d in next_d3))
                    out["central_access_after_action"] = 1.0 / (
                        1.0 + float(out["defenders_between_ball_and_goal_after_action"])
                    )
                    next_box = opp_next[
                        (opp_next["x"] >= BOX_X_MIN) & (opp_next["y"].between(BOX_Y_MIN, BOX_Y_MAX))
                    ]
        ...
        if _is_pass_event(row):
            ...
            if not np.isnan(rx) and not np.isnan(ry):
                receiver_opp_dist = [
                    _distance(rx, ry, float(x), float(y))
                    for x, y in zip(opponents["x"], opponents["y"], strict=False)
                ]
                out["nearest_defender_to_receiver"] = (
                    min(receiver_opp_dist) if receiver_opp_dist else np.nan
                )
                ...
                for _, opp in opponents.iterrows():
                    d = _point_to_segment_distance(
                        float(opp["x"]), float(opp["y"]), ball_x, ball_y, rx, ry
                    )
                    if d <= 1.0:
                        lane_blocks_pass += 1
                out["passing_lane_blockage_proxy"] = lane_blocks_pass
                out["defensive_line_height"] = (
                    float(opponents["x"].max()) if not opponents.empty else np.nan
                )
