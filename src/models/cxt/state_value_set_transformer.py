"""SetTransformer state-value model for CxT.

Mirrors ``SetTransformerCxGModel`` but performs regression on the
discounted possession-CxG target with Huber loss, matching the loss used
by ``FFNNStateValueModel``.

API mirrors the other CxT models so it slots into ``StateValueLadder``:

    SetTransformerStateValueModel().fit(actions_df, target_col="possession_cxg")
    SetTransformerStateValueModel().predict(actions_df)
"""
...
class SetTransformerStateValueModel(_BaseStateValueModel, TorchModelMixin):
    """SetTransformer-over-freeze-frames + tabular fusion for CxT regression."""
    ...
    def _frames(self) -> pd.DataFrame:
        if self._frames_cache is None:
            ...
            logger.info(
                "SetTransformerStateValue: loaded %d freeze-frame rows",
    ...
    def _build_torch_model(self, tabular_dim: int):
        ...
        head = build_mlp_head(
            ...
            out_dim=1,
            dropout=self.dropout,
            final_activation="softplus",  # ensure non-negative regression output
        )

        class _SetTransformerStateValue(nn.Module):
            def __init__(self):
                super().__init__()
        ...
        return _SetTransformerStateValue()
    ...
    def fit(
        ...
        logger.info(
            "SetTransformerStateValue: %d / %d actions have ≥1 freeze-frame row",
            n_with,
            len(df),
        )
        ...
        for epoch in range(self.max_epochs):
            ...
            if (epoch + 1) % 5 == 0 or epoch == 0:
                logger.info(
                    "SetTransformerStateValue epoch %d/%d loss=%.4f",
                    epoch + 1,
                    self.max_epochs,
    ...
    def predict(self, actions_df: pd.DataFrame) -> np.ndarray:
        ...
        with torch.no_grad():
            out = (
                self._torch_model(tokens.to(device), mask.to(device), X_tab.to(device))
                .cpu()
                .numpy()
            )
    ...
    @classmethod
    def load(cls, path: str | Path) -> SetTransformerStateValueModel:
