...
from analysis._utils import load_events, load_shots, save_json
...
def _record(name: str, statistic: float, pval: float, n_hypotheses: int, effect_size: float,
            effect_label: str = "effect_size", extra: dict | None = None) -> dict:
...
def run_hypotheses(shots: pd.DataFrame, events: pd.DataFrame) -> list[dict]:
    ...
    if bp_col:
        ...
        results.append(_record("H1_header_vs_foot_goal_rate", stat, pval, N, cohen,
                                "cohen_d", {"mean_head": round(head.mean(), 4),
                                            "mean_foot": round(foot.mean(), 4)}))
    else:
    ...
    if x_col and y_col and xg_col:
        ...
        if len(inside) > 5 and len(outside) > 5:
            ...
            effect = float((inside.mean() - outside.mean()) / shots_valid[xg_col].std()) if shots_valid[xg_col].std() > 0 else float("nan")
            results.append(_record("H2_inside_box_higher_xg", stat, pval, N, effect,
                                    "cohen_d_approx", {"mean_inside": round(inside.mean(), 4),
    ...
    if sp_col and "goal" in shots.columns:
        ...
        is_sp = sp_str.isin(["corner", "free_kick", "penalty", "direct_free_kick", "indirect_free_kick", "true", "1"])
        a = sp[is_sp]["goal"]
        b = sp[~is_sp]["goal"]
        if len(a) > 5 and len(b) > 5:
            stat, pval = stats.ttest_ind(a, b, equal_var=False)
            results.append(_record("H3_setpiece_vs_openplay_goal_rate", stat, pval, N,
                                    _cohen_d(a, b), "cohen_d",
    ...
    if seq_col and xg_col and xg_col in shots.columns:
        ...
        if len(long_seq) > 5 and len(short_seq) > 5:
            ...
            results.append(_record("H4_long_sequence_xg", stat, pval, N,
                                    _cohen_d(long_seq, short_seq), "cohen_d",
    ...
    if ha_col and "goal" in shots.columns:
        ...
        if len(home) > 5 and len(away) > 5:
            ...
            results.append(_record("H5_home_vs_away_goal_rate", stat, pval, N,
                                    _cohen_d(home, away), "cohen_d",
                                    {"mean_home": round(home.mean(), 4),
                                     "mean_away": round(away.mean(), 4)}))
    ...
    if "has_360" in shots.columns and xg_col and xg_col in shots.columns:
        ...
        if len(with360) > 5 and len(without360) > 5:
            ...
            results.append(_record("H6_360_vs_non360_xg", stat, pval, N,
                                    _cohen_d(with360, without360), "cohen_d",
            ...
            if len(groups_ss) >= 2:
                ...
                if combined.nunique() > 1:
                    ...
                    if not np.isnan(stat) and not np.isnan(pval):
                        ...
                        results.append(_record("H7_score_state_goal_rate", stat, pval, N, eta2, "eta_squared",
                                                {"n_groups": n_groups,
                                                 "group_means": {str(k): round(float(v.mean()), 4)
    ...
    if def_col and "goal" in shots.columns:
        ...
        if len(goal_def) > 5 and len(no_goal_def) > 5:
            ...
            results.append(_record("H8_defender_distance_vs_goal", stat, pval, N,
                                    _cohen_d(goal_def, no_goal_def), "cohen_d",
